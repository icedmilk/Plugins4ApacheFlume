...
public class ReliableSpoolingFileEventReader implements ReliableEventReader {
	...
	public List<Event> readEvents(int numEvents) throws IOException// TO DO
																	// LUCHENG
	{
		...
		do {
			...
			String currentName  = currentFile.get().getFile().getAbsolutePath();
//			logger.debug(nextName + " " + currentName + " " + (nextName.equals(currentName)?"true" : "false"));
		...
		// try {
		// Thread.sleep(500);// Sleep to wait data
		// } catch (InterruptedException e) {
		// // TODO Auto-generated catch block
		// e.printStackTrace();
		// }
	...
	private void rollCurrentFile(File fileToRoll) throws IOException {
		...
		if (dest.exists() && PlatformDetect.isWindows()) {
			...
			// {
			// String message =
			// "File name has been re-used with different"//lucheng flag
...
//		logger.debug(file);
		...
		do{//lucheng, choose file algorithm
		int num = rd.nextInt(candidateFiles.size());//TO DO LUCHENG2
		rdFile = candidateFiles.get(num); // Select the first random3
		selectedFile = candidateFiles.get(0); // Select the first random3
													// file.
...
//			logger.info("\n\n"+rdFile.getAbsolutePath()+" " + selectedFile.getAbsolutePath()+ " "+ candidateFiles.size()+"\n\n");
//		} else { // default order is OLDEST
		}while(rdFile.getAbsolutePath().equals(selectedFile.getAbsolutePath()) && candidateFiles.size() > 2);
...
if(candidateFiles.size() == 2)
	return openFile(selectedFile);
		...
		// return openFile(new
		// File("/home/lucheng/Desktop/httpd-2.4.10/logs/ne"));
	}
	 ...
	 */
	private Optional<FileInfo> openFile(File file)// lucheng open file
	{
		try {
			...
			EventDeserializer deserializer = EventDeserializerFactory
					.getInstance(deserializerType, deserializerContext, in);// deserial
																			// lucheng
	...
	public static class Builder {
		...
		private DecodeErrorPolicy decodeErrorPolicy = DecodeErrorPolicy
				.valueOf(SpoolDirectorySourceConfigurationConstants.DEFAULT_DECODE_ERROR_POLICY
						.toUpperCase());
		...
		public ReliableSpoolingFileEventReader build() throws IOException {
			return new ReliableSpoolingFileEventReader(spoolDirectory,
					...
					deletePolicy, inputCharset, decodeErrorPolicy, consumeOrder);
		}
	}
